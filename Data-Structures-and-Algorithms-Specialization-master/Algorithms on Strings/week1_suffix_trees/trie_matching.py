...
Input Format.
	...
	Constraints. 1 ≤ |Text| ≤ 10 000; 1 ≤ n ≤ 5 000; 1 ≤ |p i | ≤ 100 for all 1 ≤ i ≤ n; all strings contain only
	symbols A, C, G, T; no p i is a prefix of p j for all 1 ≤ i ̸ = j ≤ n.
...
def solve(text, patterns):
	...
	def prefix_trie_matching(text):
		...
		while True:
			if not current_branch:
				return True # Comes here when the pattern is read due to defaultdict
			elif symbol in current_branch:
	...
	for index in range(len(text)):
		if prefix_trie_matching(text[index:]):
			result.append(index)
